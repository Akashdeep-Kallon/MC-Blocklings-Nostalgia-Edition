...
public class OrderedItemInfoSet implements Iterable<ItemInfo>
{
    ...
    {
        boolean wasRemoved = itemInfos.remove(itemInfo);
    ...
    public void set(int index, @Nonnull ItemInfo itemInfo, boolean sync)
    {
        ...
        // Prefer caller-provided index when it is valid; this is authoritative for UI edits
        // and avoids dropping updates if decoded item identity cannot be matched.
        if (index >= 0 && index < itemInfos.size())
        {
            realIndex = index;
        }
        else
        {
            // Fallback for stale indexes: resolve by item identity.
            for (int i = 0; i < itemInfos.size(); i++)
            {
                if (itemInfos.get(i).equals(itemInfo))
                {
                    realIndex = i;
                    break;
                }
        ...
        if (realIndex < 0)
        {
            // Fallback: if not found, treat this as an add so edits are never silently lost.
            add(itemInfo, false, sync);
            return;
        }

        ItemInfo oldItemInfo = itemInfos.get(realIndex);

        itemInfos.set(realIndex, itemInfo);

        if (sync)
        {
            new SetItemInfoInfoMessage(itemSetProvider, itemInfo, realIndex).sync();
        }

        eventBus.post(this, new ItemInfoSetEvent(oldItemInfo, itemInfo, realIndex));
    }
