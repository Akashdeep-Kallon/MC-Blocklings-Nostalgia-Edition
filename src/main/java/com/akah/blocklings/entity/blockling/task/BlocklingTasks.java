...
import com.akah.blocklings.entity.blockling.goal.goals.combat.BlocklingMeleeAttackHurtByGoal;
import com.akah.blocklings.entity.blockling.goal.goals.combat.BlocklingMeleeAttackOwnerHurtByGoal;
import com.akah.blocklings.entity.blockling.goal.goals.combat.BlocklingMeleeAttackOwnerHurtGoal;
import com.akah.blocklings.entity.blockling.goal.goals.container.BlocklingDepositContainerGoal;
import com.akah.blocklings.entity.blockling.goal.goals.container.BlocklingTakeContainerGoal;
import com.akah.blocklings.entity.blockling.goal.goals.gather.BlocklingFarmGoal;
import com.akah.blocklings.entity.blockling.goal.goals.gather.BlocklingMineGoal;
import com.akah.blocklings.entity.blockling.goal.goals.gather.BlocklingWoodcutGoal;
import com.akah.blocklings.entity.blockling.goal.goals.misc.*;
...
import com.akah.blocklings.network.messages.TaskCreateMessage;
import com.akah.blocklings.network.messages.TaskRemoveMessage;
import com.akah.blocklings.network.messages.TaskTypeIsUnlockedMessage;
...
import com.akah.blocklings.util.event.HandleableEvent;
...
public class BlocklingTasks implements IReadWriteNBT
{
    ...
    {
        clearSelector(goalSelector);
        clearSelector(targetSelector);

        goalSelector.addGoal(0, new FloatGoal(blockling));
        goalSelector.addGoal(1, new BlocklingWildPanicGoal(blockling, 1.25D));
        ...
        for (Task task : prioritisedTasks)
        {
            ...
            if (goal.getFlags().contains(Goal.Flag.TARGET))
            {
                targetSelector.addGoal(task.getPriority(), goal);
            }
            else
            {
    ...
    private static void clearSelector(@Nonnull GoalSelector selector)
    {
        Set<WrappedGoal> wrappedGoals = new HashSet<>(selector.getAvailableGoals());
    ...
    {
        UUID resolvedTaskId = taskId != null ? taskId : UUID.randomUUID();

        // [final-release-1.20.1] motivo: fuerza una sola tarea por tipo para mantener consistencia UI/IA.
        if (type != NULL && hasTaskOfType(type))
        {
            return;
        }

        if (getTask(resolvedTaskId) == null)
        {
            Task task = new Task(resolvedTaskId, type, blockling, this);
            ...
            if (sync)
            {
                new TaskCreateMessage(blockling, type.id, resolvedTaskId).sync();
            }
        }
    }


    public boolean hasTaskOfType(@Nonnull TaskType type)
    {
        return prioritisedTasks.stream().anyMatch(task -> task.getType() == type);
    }

    public boolean hasTaskOfType(@Nonnull TaskType type, @Nonnull UUID excludedTaskId)
    {
        return prioritisedTasks.stream().anyMatch(task -> task.getType() == type && !task.id.equals(excludedTaskId));
    }

    public boolean hasEnabledTaskOfType(@Nonnull TaskType type)
    {
        return prioritisedTasks.stream()
                .filter(Task::isConfigured)
                .anyMatch(task -> task.getType() == type && task.getGoal() != null && task.getGoal().getState() != BlocklingGoal.State.DISABLED);
    }
