...
import com.akah.blocklings.util.ToolContext;
import com.akah.blocklings.util.ToolType;
...
public abstract class BlocklingMeleeAttackGoal extends BlocklingTargetGoal<LivingEntity>
{
    ...
    public BlocklingMeleeAttackGoal(@Nonnull UUID id, @Nonnull BlocklingEntity blockling, @Nonnull BlocklingTasks tasks)
    {
        ...
        EntityUtil.getValidAttackTargets().keySet().forEach(type -> whitelist.put(type, true));
    ...
    public void tickGoal()
    {
        ...
        if (isInRange(target))
        {
            BlocklingHand attackingInteractionHand = blockling.getEquipment().findAttackingHand();
            attackingInteractionHand = attackingInteractionHand == BlocklingHand.BOTH ? blockling.getActions().attack.getRecentHand() == BlocklingHand.OFF ? BlocklingHand.MAIN : BlocklingHand.OFF : attackingInteractionHand;

            if (blockling.getActions().attack.tryStart(attackingInteractionHand))
            {
                attack(target, attackingInteractionHand);
            }
        }
    }
