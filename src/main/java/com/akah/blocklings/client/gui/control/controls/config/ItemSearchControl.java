...
import com.akah.blocklings.client.gui.control.controls.TextFieldControl;
import com.akah.blocklings.client.gui.control.controls.TexturedControl;
import com.akah.blocklings.client.gui.control.controls.panels.FlowPanel;
...
import com.akah.blocklings.client.gui.control.event.events.TextChangedEvent;
...
class ItemSearchControl extends GridPanel
{
    ...
    // [fix] motivo: difiere la carga inicial hasta que el control ya fue medido/renderizado al abrirse.
    private boolean shouldRefreshOnNextTick = false;
    ...
    public void setVisibility(@Nonnull Visibility visibility)
    {
        ...
        {
            // [fix] motivo: evita poblar antes de layout; FlowPanel necesita tamaño válido para posicionar ítems.
            shouldRefreshOnNextTick = true;
        }
    }
    ...
    public void onTick()
    {
        ...
        if (shouldRefreshOnNextTick && getVisibility() == Visibility.VISIBLE)
        {
            // [fix] motivo: ejecuta la carga inicial un frame después para asegurar medidas correctas.
            shouldRefreshOnNextTick = false;
            updateItems("");
        }
    }
    ...
    private void updateItems(@Nonnull String searchText)
    {
        ...
        for (Item item : items)
        {
            ...
            if (!normalizedItemName.contains(normalizedSearch))
            {
                continue;
            }
            ...
            if (itemCount >= maxItems && !normalizedItemName.equals(normalizedSearch))
            {
                missingItems++;
        ...
        for (Item item : itemsToAdd.stream().sorted((item1, item2) -> new ItemStack(item1).getHoverName().getString().compareToIgnoreCase(new ItemStack(item2).getHoverName().getString())).collect(Collectors.toList()))
        {
            ItemControl itemControl = new ItemControl()
            {
                ...
                {
                    setForegroundColour(0xffffffff); // [fix] motivo: color blanco opaco representa "sin tinte" y evita iconos negros.
                }
