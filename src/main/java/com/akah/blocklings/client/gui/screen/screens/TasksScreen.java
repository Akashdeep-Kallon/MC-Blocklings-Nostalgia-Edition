...
import com.akah.blocklings.client.gui.control.controls.tasks.TaskControl;
import com.akah.blocklings.client.gui.control.event.events.FocusChangedEvent;
import com.akah.blocklings.client.gui.control.event.events.ReorderEvent;
import com.akah.blocklings.client.gui.control.event.events.SelectionChangedEvent;
import com.akah.blocklings.client.gui.control.event.events.TabChangedEvent;
...
import com.akah.blocklings.client.gui.properties.Direction;
...
import com.akah.blocklings.entity.blockling.task.TaskType;
...
{
    // [refactor-1.20.1] motivo: reemplaza printStackTrace por logging estructurado en cliente.
    private static final Logger LOGGER = LogManager.getLogger(TasksScreen.class);
    ...
    private void initConfigTabs(@Nonnull Task task)
    {
        ...
        BaseControl miscContainer = configTabbedPanel.addTab(new BlocklingsComponent("task.ui.tab.misc"));
        ...
        if (task.isConfigured())
        {
            ...
            for (Property property : goal.properties)
            {
                ...
                if (goal instanceof BlocklingContainerGoal)
                {
                    BlocklingContainerGoal containerGoal = (BlocklingContainerGoal) goal;

                    if (property == containerGoal.transferModeProperty || property == containerGoal.itemConfigurationTypeProperty)
                    {
                        continue;
                    }
                }
    ...
    private void onTaskTypeChanged(@Nonnull Task.TypeChangedEvent typeChangedEvent)
    {
        ...
        {
            // [refactor-1.20.1] motivo: conserva trazabilidad sin salida directa a stderr.
    ...
    public void closeConfig()
    {
        ...
        if (currentTask != null)
        {
            // [refactor-1.20.1] motivo: corrige suscripción acumulada al cerrar configuración y evita fugas de callbacks.
