...
import com.akah.blocklings.client.gui.control.event.events.SizeChangedEvent;
...
public class TextFieldControl extends Control
{
    ...
    /**
     * Guards against recursive responder -> setText -> setValue -> responder loops.
     */
    private boolean isUpdatingText = false;

    /**
     * Tracks the last known text value independently of the widget mutation timing.
     */
    @Nonnull
    ...
    public TextFieldControl()
    {
        ...
        previousText = textFieldWidget.getValue();
        textFieldWidget.setResponder((newText) ->
        {
            if (isUpdatingText)
            {
            ...
            eventBus.post(this, new TextChangedEvent(oldText, newText));
        });
...
//            Blocklings.LOGGER.warn(ex.toString());
        ...
        g.pose().mulPoseMatrix(textFieldMatrixStack.last().pose());
        textFieldWidget.render(g, (int) Math.round(mouseX), (int) Math.round(mouseY), partialTicks);
