package com.akah.blocklings.client.gui.control.controls.panels;
...
public class TabbedPanel extends Control
{
     ...
     *
     * @param index the 0-based position to insert at (clamped to valid range).
     * @param name the tab's display name.
     * @return the content container for the new tab.
     */
    @Nonnull
    public BaseControl insertTabAt(int index, @Nonnull Component name)
    {
        int clampedIndex = Math.max(0, Math.min(index, tabContainer.getChildren().size()));

        TabControl tab = new TabControl(name);
        tabContainer.insertChildAt(tab, clampedIndex);

        Control container = new Control();
        container.setWidthPercentage(1.0);
        container.setHeightPercentage(1.0);
        container.setVisibility(Visibility.COLLAPSED);
        containerContainer.insertChildAt(container, clampedIndex);
