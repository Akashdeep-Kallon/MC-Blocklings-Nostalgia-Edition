...
public class SingleSelectorStrip<O> extends Control
{
    ...
    private class OptionControl extends TexturedControl
    {
        ...
        protected void onRender(@Nonnull PoseStack matrixStack, @Nonnull ScissorStack scissorStack, double mouseX, double mouseY, float partialTicks)
        {
            ...
            // [fix] Show a disabled overlay when the strip is non-interactive,
            // so the player understands the buttons cannot be clicked.
            if (!isInteractive())
            {
                renderRectangleAsBackground(matrixStack, 0x88000000);
            }
        }
