...
import com.akah.blocklings.client.gui.control.controls.*;
...
public class ContainerControl extends GridPanel
{
     ...
     */
    @Nonnull
    private final ItemControl blockIcon;

    /**
     * The block side priority selector.
     */
    @Nonnull
    private final FlatBlockSideSelectionControl sidePriority;
    ...
    public ContainerControl(@Nonnull ContainerInfo containerInfo)
    {
        ...
        blockIcon = new ItemControl();
        iconBackground.addChild(blockIcon);
        blockIcon.setWidthPercentage(1.0);
        blockIcon.setHeightPercentage(1.0);
        blockIcon.setVerticalAlignment(0.5);
        blockIcon.setHorizontalAlignment(0.5);
        // [fix] Match item scale used by EntryControl in the whitelist (0.95 within 26/30 area).
        blockIcon.setItemScale(0.72f);
        blockIcon.setItemStack(new ItemStack(containerInfo.getBlock().asItem()));
        ...
        TexturedControl nameBackground = new TexturedControl(Textures.Common.BAR_RAISED)
        {
            ...
            protected void onRender(@Nonnull PoseStack matrixStack, @Nonnull ScissorStack scissorStack, double mouseX, double mouseY, float partialTicks)
            {
                ...
                // [final-release-1.20.1] motivo: restaura estado global de color para evitar artefactos de render en iconos.
        ...
        dropdownGrid.addColumnDefinition(GridDefinition.RATIO, 0.75);
        ...
        sidePriority = new FlatBlockSideSelectionControl()
        {
            @Override
            public void onRenderTooltip(@Nonnull PoseStack matrixStack, double mouseX, double mouseY, float partialTicks)
            {
                Direction mouseOverDirection = getDirectionMouseIsOver();
                Component name = new BlocklingsComponent("config.container.side_priority.name").copy();
                ...
                if (mouseOverDirection != null)
                {
                    ...
                    {
                        case NORTH:
                            name = name.copy().append(new BlocklingsComponent("config.container.side_priority.side", new BlocklingsComponent("direction.front")));
                            break;
                        case SOUTH:
                            name = name.copy().append(new BlocklingsComponent("config.container.side_priority.side", new BlocklingsComponent("direction.back")));
                            break;
                        case WEST:
                            name = name.copy().append(new BlocklingsComponent("config.container.side_priority.side", new BlocklingsComponent("direction.left")));
                            break;
                        case EAST:
                            name = name.copy().append(new BlocklingsComponent("config.container.side_priority.side", new BlocklingsComponent("direction.right")));
                            break;
                        case UP:
                            name = name.copy().append(new BlocklingsComponent("config.container.side_priority.side", new BlocklingsComponent("direction.top")));
                            break;
                        case DOWN:
                            name = name.copy().append(new BlocklingsComponent("config.container.side_priority.side", new BlocklingsComponent("direction.bottom")));
                            break;
                    }
                }

                List<FormattedCharSequence> tooltip = new ArrayList<>();
                tooltip.add(name.copy().withStyle(ChatFormatting.WHITE).getVisualOrderText());
        ...
        sidePriority.setWidthPercentage(0.9);
        sidePriority.setHeightPercentage(0.9);
        sidePriority.setHorizontalAlignment(0.5);
        sidePriority.setVerticalAlignment(0.5);
        sidePriority.setDebugName("Side Priority");
        sidePriority.setMarginTop(4.0);
        sidePriority.setMarginBottom(4.0);
        sidePriority.setMarginLeft(4.0);
        sidePriority.setMarginRight(4.0);
        sidePriority.setBlock(containerInfo.getBlock());
        sidePriority.setSelectedDirections(containerInfo.getSides());
        sidePriority.eventBus.subscribe((BaseControl c, FlatBlockSideSelectionControl.DirectionListChangedEvent e) ->
        {
            ContainerInfo oldContainerInfo = new ContainerInfo(containerInfo);
    ...
    public void setBlock(@Nonnull Block block)
    {
        ...
        currentBlock = block;
        blockIcon.setItemStack(new ItemStack(block.asItem()));
    ...
    {
        event.setCanceled(handleContainerSelect(event.getEntity(), event.getHand() == InteractionHand.OFF_HAND, event.getPos()));
    }
    ...
    {
        event.setCanceled(handleContainerSelect(event.getEntity(), true, null));
    }
    ...
    {
        handleContainerSelect(event.getEntity(), true, null);
    }
}
